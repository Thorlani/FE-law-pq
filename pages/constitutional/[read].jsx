...
export async function getStaticPaths() {
  try {
    const result = await axios.get(
      "https://be-law-pq.vercel.app/api/getConstitutional"
    );
...
export async function getStaticProps(context) {
  ...
  try {
    const res = await axios.get(
      "https://be-law-pq.vercel.app/api/getConstitutional/" + read
    );
...
>>>>>>> 8948958 (removed the comment section of the app)
  } catch (error) {
    console.log(error);
  }
}
...
const ConstitutionalRead = ({ data, comments }) => {
  ...
  const postComment = async (event) => {
    ...
    try {
      ...
      axios
        .post("https://be-law-pq.vercel.app/api/postCLC", body)
        .then((res) => {
          setIsLoading(false);
        ...
        <div className="w-full h-fit p-5 shadow-xl">
          ...
          {/* <div className="mt-[50px] w-full h-fit px-[2%]">
            <h2 className="font-semibold mb-5">
              How did you find the answer helpful?
            </h2>
            <form onSubmit={postComment}>
              ...
              <button
                type="submit"
                className="mt-[10px] px-[15px] py-[6px] bg-black text-white rounded-md"
              >
                Post
              </button>
            </form>
