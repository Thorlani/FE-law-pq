...
export async function getStaticPaths () {
  ...
  try {
    const result = await axios.get('https://be-law-pq.vercel.app/api/getConstitutional');
...
export async function getStaticProps (context) {
  ...
  try {
    const res = await axios.get('https://be-law-pq.vercel.app/api/getConstitutional/' + read)
    const result = await axios.get('https://be-law-pq.vercel.app/api/getCLC');
...
const ConstitutionalRead = ({ data, comments }) => {

  const commentSection = comments.map((comment) => {
    return (
      <>
        <div key={comment.name} className="w-full md:w-[60%] h-fit flex items-start mb-3">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 mt-[6px]">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
          </svg>
          <div className="ml-5">
            <span className="mb-2 font-bold">{comment.name}</span>
  ...
  const postComment = async (event) => {
    ...
    try {
      ...
      axios.post('https://be-law-pq.vercel.app/api/postCLC', body)
      .then(res => {
        setIsLoading(false)
...
export default ConstitutionalRead